...
namespace april::container {
	...
	namespace internal {
		...
		class LinkedCells final : public ContiguousContainer<container::LinkedCells, U> {
			...
			struct CellPair {
				const uint32_t c1 = {}; // start index of first particle in cell c1
				const uint32_t c2 = {}; // start index of first particle in cell c2
			};
			...
			struct AsymmetricBatch : BatchBase<BatchSymmetry::asymmetric, false> {
				std::span<const size_t> type1_indices{};
				std::span<const size_t> type2_indices{};
			};

			struct SymmetricBatch : BatchBase<BatchSymmetry::symmetric, false> {
				std::span<const size_t> type_indices{};
			};
		...
		public:
			...
			void build(const std::vector<ParticleRecord> & input_particles) {
				this->init_storage(input_particles);
				set_cell_size();
		...
		private:
			...
			void rebuild_cell_structure() {
				...
				for (size_t i = 0; i < N; ++i) {
					...
					tmp_i[dst] = id_to_index_map[i];
				}
				particles.swap(tmp_p);
				id_to_index_map.swap(tmp_i);
			}
