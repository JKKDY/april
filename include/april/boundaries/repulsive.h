...
namespace april::boundary {
	...
	template<typename T>
	concept IsScalarForce =
	requires(const T t, double d) {

		// 1. Check for the cutoff() method
		{ t.cutoff() } noexcept -> std::same_as<double>;

		// 2. Check for the apply() method.
		// We test the expression by 'declval'-ing a canonical
		// IsMutableFetcher type.
		{ t.apply(std::declval<env::internal::ParticleRecordFetcher<env::NoUserData>&>(), d) } noexcept
		-> std::same_as<double>;
	};
	...
	struct ExponentialForce {
        ...
        template<env::IsMutableFetcher F>
		[[nodiscard]] double apply(F &, const double distance) const noexcept {
