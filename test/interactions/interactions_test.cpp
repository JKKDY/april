...
using namespace april::interaction;
using namespace april::particle;
using namespace april::particle::internal;
using namespace april;


struct InteractionTestUserData {
    double charge = 0.0;
    bool operator==(const InteractionTestUserData&) const = default;
};


static_assert(particle::IsParticleAttributes<InteractionTestUserData>, "Test data must satisfy IsUserData");
...
protected:
    vec3 pos1, pos2;
    double mass1{}, mass2{};
    InteractionTestUserData data1, data2;
    ...
    [[nodiscard]] auto make_source(
        const vec3& position,
        const double& mass,
        const InteractionTestUserData& attributes
    ) const {
...
// --- Interaction Tests ---
// Verifies that NoInteraction consistently returns a zero vector and
// that mixing two NoInteraction objects preserves this behavior.
TEST_F(InteractionTest, NoInteractionTest) {
    NoInteraction f;
...
TEST_F(InteractionTest, GravityTest) {
    Gravity g(1.0);
...
TEST_F(InteractionTest, HarmonicTest) {
...
TEST_F(InteractionTest, CoulombTest) {
    Coulomb c(1.0);
...
TEST_F(InteractionTest, LennardJonesTest) {
...
class InteractionMixingTest : public testing::Test {};
...
// - Cutoff: Maximum
TEST_F(InteractionMixingTest, Harmonic_SeriesMixing) {
...
TEST_F(InteractionMixingTest, Gravity_Mixing_Mismatch_Throws) {
    // Universe A: G=10
...
TEST_F(InteractionMixingTest, Gravity_Mixing_Success) {
...
// - Cutoff: Maximum
TEST_F(InteractionMixingTest, Coulomb_Mixing_Mismatch_Throws) {
    // Medium A: ke=100
...
TEST_F(InteractionMixingTest, Coulomb_Mixing_Success) {
...
TEST_F(InteractionMixingTest, LennardJones_LorentzBerthelot) {
