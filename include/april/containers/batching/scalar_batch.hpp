...
namespace april::container::batching {
	...
	struct AsymmetricScalarBatch : BatchBase<exec::internal::ParallelTrait::None, exec::internal::VectorTrait::ScalarOnly> {
		...
		void for_each_pair (Kernel && f) const {
			...
			container.template for_each_particle<P, VectorPolicy::Scalar> (range1.start, range1.stop, scalar_kernel<K::access, K::update>(
				[&]<bool is_packed1>(auto && p1) {
					container.template for_each_particle<P,  VectorPolicy::Scalar> (range2.start, range2.stop, scalar_kernel<K::access, K::update>(
	...
	struct SymmetricScalarBatch : BatchBase<exec::internal::ParallelTrait::None, exec::internal::VectorTrait::ScalarOnly> {
		...
		void for_each_pair (Kernel && f) const {
			using K = std::remove_cvref_t<Kernel>;
			container.template for_each_particle<P, VectorPolicy::Scalar> (range.start, range.stop, scalar_kernel<K::access, K::update>(
				[&]<bool is_packed1>(const size_t i, auto && p1) {
					container.template for_each_particle<P, VectorPolicy::Scalar> (i + 1, range.stop, scalar_kernel<K::access, K::update>(
						[&]<bool is_packed2>(auto && p2) {
							f(p1, p2);
						})
					);
