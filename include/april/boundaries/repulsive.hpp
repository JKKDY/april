...
namespace april {
	...
	namespace  boundary {
		...
		requires(const T t, double d) {
			...
			{ t.apply(d) } noexcept -> std::same_as<double>;
		};
	}
	...
	struct WallForce {
		struct ExponentialForce {
			...
			[[nodiscard]] double apply(const double distance) const noexcept {
	...
	struct RepulsiveBoundary : boundary::Boundary {
		...
		void apply(particle::internal::ScalarParticleRef<M, U> & particle, const core::Box & domain_box, const DomainFace face) const noexcept{
			...
			const double magnitude = boundary_force.apply(distance );
			const int direction = is_plus ? -1 : 1;  // push inward
